<div>
    ...
    $title = $editing ? join(" ", [ __('Edit'), __('TerminationType'), $terminationtype->name]) : join(" ",
    [__('Create'),
    ...
    <x-human_resource::modal modal-name="TerminationTypeForm" title="{{ $title }}"
        ...
        <x-human_resource::form :editing="$editing">
            <div class="p-3">
                <x-human_resource::inputs.with-labels field="terminationtype.name">Name:
                </x-human_resource::inputs.with-labels>

                <x-human_resource::inputs.text-area field="terminationtype.description" :required="false">{{
                    __('Description') }}:
